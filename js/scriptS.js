...
function setupCityListeners() {
    // Välj alla länkar med antingen data-stad eller data-provins
    const navLinks = document.querySelectorAll(".nav-menu a[data-stad], .nav-menu a[data-provins]");
    
    // Lägg till en klicklyssnare för varje länk
    navLinks.forEach(link => {
        link.addEventListener("click", function (event) {
            event.preventDefault(); // Förhindra navigering

            // Toggla aktiv status på länken
            this.classList.toggle("active");

            // Endast anropa `filterAndShow` om länken är aktiv
